...
async def sync_iiko_menu_to_rag(
    ...
    store = get_app_store()
    from app.schemas import KnowledgeSourceCreateRequest

    payload = KnowledgeSourceCreateRequest(
        title="iiko Menu Sync",
        source_type="text",
        content=content,
    )
    source = store.create_knowledge_source(UUID(tenant_id), payload)

    # 4. Get ingestion job
    job = store.enqueue_knowledge_ingestion(UUID(tenant_id), source.id)
    job_id = str(job.id) if job else ""

    return {"status": "accepted", "job_id": job_id, "source_id": str(source.id)}
